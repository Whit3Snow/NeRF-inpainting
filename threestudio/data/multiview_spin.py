...
class MultiviewIterableDataset(IterableDataset):
    def __init__(self, cfg: Any) -> None:
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            frame_path = os.path.join(self.cfg.dataroot, frame["file_path"].split('/',2)[-1])
            img = cv2.imread(frame_path)[:, :, ::-1].copy()
            img = cv2.resize(img, (int(self.frame_w), int(self.frame_h)))
...
class MultiviewDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        self.frame_w = int(camera_dict["w"] // scale)
        self.frame_h = int(camera_dict["h"] // scale)
        threestudio.info("Loading frames...")
        self.n_frames = len(frames)
